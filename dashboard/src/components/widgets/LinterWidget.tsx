...
export const LinterWidget = () => {
  ...
  useEffect(() => {
    const fetchConfig = async () => {
      try {
        const response = await fetch(`${API_URL}/api/linter/config`, {
          headers: {
            'Accept': 'application/json'
          }
        })
        if (!response.ok) throw new Error('Failed to fetch linter config')
        const data = await response.json()
        setConfig(data)
        setConfigError(null)
      } catch (err) {
        console.error('Failed to fetch linter config:', err)
  ...
  const handleConfigChange = async (updates: Partial<LinterConfig>) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error('Failed to update linter config')
      await refetch()
      setConfigError(null)
    } catch (err) {
